...
var hsvToRgb16 = function (hue, saturation, value) {
  var result = false;
  if (((saturation || saturation === 0) && saturation <= 100) && ((value || value === 0) && value <= 100)) {
    var red = 0,
      green = 0,
      blue = 0,
      i = 0,
...
function drawhoneycomb() {
  for (let i = 0; i < RainbowHoneycomb.width; ++i) {
    draw.fillStyle = hsvToRgb16(i * 360 / RainbowHoneycomb.width + tickcount, 100, 100);
  ...
  for (let i = 0; i < Math.round(RainbowHoneycomb.width / radius) + 2; ++i) {
    for (let j = 0; j < Math.round(RainbowHoneycomb.height / radius) + 2; ++j) {
      honeycomb(radius * 2 * i + (j % 2) * radius, radius * 2 * root3 / 2 * j, radius);
    }
  }
  tickcount += 1;
}
